...
import { Youtube, Play, X, Loader2, AlertCircle, ExternalLink, ArrowLeft } from 'lucide-react';
...
export default function DrackerVideoGallery() {
    ...
    useEffect(() => {
        const handleSelectPlaylist = (e) => {
            setPlaylistId(e.detail);
        };
        window.addEventListener('select-dracker-playlist', handleSelectPlaylist);
        return () => window.removeEventListener('select-dracker-playlist', handleSelectPlaylist);
    }, []);
    ...
    const fetchPlaylistsData = useCallback(async () => {
        ...
        setError(null);
        try {
            const ids = DRACKER_CONFIG.PLAYLISTS.map(p => p.id).join(',');
            const response = await fetch(`https://www.googleapis.com/youtube/v3/playlists?part=snippet&id=${ids}&key=${apiKey}`);
            const data = await response.json();
            if (data.error) throw new Error(`${data.error.message}`);
            
            const sorted = [];
            DRACKER_CONFIG.PLAYLISTS.forEach(p => {
                const item = data.items?.find(i => i.id === p.id);
                if (item) sorted.push(item);
            });
            setPlaylistsInfo(sorted);
        } catch (err) {
            setError(`Erro ao carregar capas das playlists: ${err.message}`);
        } finally {
            setLoading(false);
        }
    }, [apiKey]);
    ...
    const fetchPlaylist = useCallback(async () => {
        ...
        try {
            ...
            const data = await response.json();
            if (data.error) throw new Error(`${data.error.message} (Código: ${data.error.code})`);
    ...
    useEffect(() => {
        if (playlistId) {
            fetchPlaylist();
        } else if (playlistsInfo.length === 0) {
            fetchPlaylistsData();
        }
    }, [playlistId, fetchPlaylist, fetchPlaylistsData, playlistsInfo.length]);

    const currentPlaylistTitle = useMemo(() => {
        if (!playlistId) return "Playlists do Canal Drácker";
        const playlist = DRACKER_CONFIG.PLAYLISTS?.find(p => p.id === playlistId);
        return playlist ? playlist.title : "Vídeos Educativos";
    }, [playlistId]);
            ...
            <div className="bg-white border-b border-brown-200 px-6 py-4 flex items-center justify-between shrink-0">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-brown-500 font-medium">{playlistId ? "Vídeos Pedagógicos" : "Selecione uma playlist para explorar"}</p>
                    </div>
                </div>
            </div>
                            ...
                            playlistsInfo.map((playlist) => (
                                ...
                                >
                                    <div className="relative aspect-video overflow-hidden">
                                        <img
                                            src={playlist.snippet.thumbnails?.high?.url || playlist.snippet.thumbnails?.medium?.url}
                                    ...
                                    <div className="p-5">
                                        ...
                                        <p className="text-xs text-brown-500 line-clamp-2">
                                            {playlist.snippet.description || 'Playlist de vídeos educativos do Drácker.'}
                                        </p>
                                    </div>
                                </div>
                            ))
