...
const generatePDFHTML = (member) => {
    ...
    const styles = `
        ...
        .badge { align-self: flex-start; background-color: #78350f; color: white; padding: 0 20px; height: 34px; border-radius: 20px; font-size: 16px; font-weight: bold; display: flex; align-items: center; justify-content: center; }
...
const ArchetypeBadge = ({ type, size = "md", compact = false }) => {
    ...
    return (
        <div className={`flex items-center justify-center gap-1.5 rounded-lg border ${config.color} ${sizeClasses} font-bold shadow-sm inline-flex`}>
            <Icon size={iconSize} /> <span className="leading-none -translate-y-[2px]">{type}</span>
        </div>
    );
};
...
const MemberModal = ({ member, onClose, onUpdate, onRemove }) => {
    ...
    const handleDownloadPDF = async () => {
        ...
        iframe.onload = async () => {
            try {
                // Ensure iframe height fits content to prevent truncation on mobile
                ...
                const opt = {
                    ...
                    html2canvas: { scale: 2, useCORS: true, windowWidth: 1000, scrollY: 0 },
